...
KiwiPanel gives you per-website control over PHP configuration. Each website can have its own PHP version, performance tuning, error handling, and `open_basedir` override — all managed from one page. Security directives like `disable_functions` are managed server-wide at **Settings → PHP Security**.
...
The following directives are `PHP_INI_SYSTEM` — they can only be set at the server level and apply to **all websites**. They are managed at **Settings → PHP Security** (`/settings/php-security`).

#### disable_functions

Controls which PHP functions are blocked from being called. Four presets are available:

| Preset | Description |
|--------|-------------|
| **Standard** (default) | Blocks dangerous functions like `exec`, `system`, `passthru`, `shell_exec`, `proc_open`, `popen`. Best for most websites. |
...
::: info
The per-website PHP page shows the active `disable_functions` preset and the resolved function list as a **read-only** card. To change it, click the **Manage in Settings** button which links to **Settings → PHP Security**.
:::
...
### Per-Website: open_basedir

`open_basedir` is the only security directive that can be overridden per-website. It restricts which directories PHP can access on the filesystem. When enabled, PHP can only read files within the website's home directory and a few system paths (`/tmp`, etc.).

**To customize open_basedir for a single website:**

1. Open the website's PHP settings page
2. Scroll to the **open_basedir Settings** card
3. Uncheck **Use global open_basedir defaults**
4. Toggle the `open_basedir` switch and optionally add extra allowed paths
5. Click **Save open_basedir Settings**

A badge indicates when a site has a custom override active. To revert, re-check **Use global open_basedir defaults** and save.

::: tip
All other security directives (`disable_functions`, `expose_php`, `allow_url_include`, `session.*`) are PHP_INI_SYSTEM and cannot be overridden per-website. They are enforced server-wide via **Settings → PHP Security**.
:::
...
├─────────────────────────────────┤
│ mods-available/*.ini            │  ← Global security (disable_functions,
│                                 │    expose_php, allow_url_include, session.*)
│                                 │    Managed via Settings → PHP Security
├─────────────────────────────────┤
│ OLS vhost phpIniOverride        │  ← Per-site settings (memory_limit, etc.)
│                                 │    + per-site open_basedir override
├─────────────────────────────────┤
...
**Important:** PHP_INI_SYSTEM directives (`disable_functions`, `expose_php`, `allow_url_include`, `session.cookie_httponly`, `session.cookie_secure`, `session.use_strict_mode`) are set globally via `mods-available/*.ini` and apply to all websites. They cannot be overridden per-website because PHP enforces this restriction at the engine level.

Settings applied via `php_admin_value` in the OLS vhost config (like `open_basedir`) **cannot** be overridden by `.user.ini` or `ini_set()` — this is also enforced by PHP itself.
